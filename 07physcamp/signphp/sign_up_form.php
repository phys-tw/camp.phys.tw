...
if (time() > $deadline)
  ...
  $db = new DB('localhost', 'physcamp', 'physland', 'physcamp') ;//(host, user, passwd, database, persistant[預設為false]) see db.php
  if (! $db->open()) {
    $messenge = "資料查詢失敗：無法開啟資料庫。" ;
    die($db->error()) ;
  }
  else if (! $db->query("Select * From 2k07_list Where ID = '" . $_GET['ID'] . "' Order By SignTime Desc")) {
...
<table width="580" border="0" cellspacing="0" cellpadding="0" style="margin-left: 26px">
  <tr><td>
	...
	<table width="148" height="26" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="148"  class="style2">&nbsp;填寫個人資料</td>
        </tr>
    </table>
