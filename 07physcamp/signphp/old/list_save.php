...
//  echo "</pre>" ;
  ...
  if ($_POST['list_action'] == "save") {
    for ($i = 0 ; $i < count($_POST['State']) ; $i++) {
      ...
      $p_SQL = "Update 2k07_list Set " . $p_SQL . " Where AutoNo = " . $_POST['AutoNo'][$i] ;
...
//      echo $p_SQL, "<br>" ;
    ...
    $mail_list = "" ;
    for ($i = 0 ; $i < count($_POST['State']) ; $i++) {
      $p_SQL = "Select Name, Email From 2k07_list Where AutoNo = " . $_POST['AutoNo'][$i] ;
	  if (! $db->query($p_SQL))
	    die($db->error()) ;
	...
	$k = 0 ;
    for ($i = 0 ; $i < count($_POST['State']) ; $i++) {
      $p_SQL = "Select Name, Address From 2k07_list Where AutoNo = " . $_POST['AutoNo'][$i] ;
	  if (! $db->query($p_SQL))
	    die($db->error()) ;
      if ($fetch_Assoc = $db->fetchAssoc()) {
	    if ($k == 0)
