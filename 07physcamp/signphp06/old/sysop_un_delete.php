...
if ($sysop) {
  ...
  if ($execute) {
	...
	if (! $db->open())
      die($db->error()) ;
	if (! $db->query("Select ID From 2k06_list Where AutoNo = " . $no))
      die($db->error()) ;
    $fetch_Assoc = $db->fetchAssoc() ;
	if (! $db->query("Update 2k06_list Set State = 9, Reason = '恢復較早的資料' Where ID = '" . $fetch_Assoc['ID'] . "' And State != 9"))
      die($db->error()) ;
	if (! $db->query("Update 2k06_list Set State = 0, Reason = '刪除後再恢復', SignTime = '" . date("Y-m-d H:i:s", time()) . "' Where AutoNo = " . $no))
      die($db->error()) ;
	$db->freeResult() ;
	$db->close() ;
